...
<script runat="server">
    private void Page_Load(object sender, EventArgs e)
    {
    	...
    	if (milestoneID == Guid.Empty)
    		throw new InvalidOperationException("No milestone specified by the query string.");
    		
    	if (taskID == Guid.Empty)
    		throw new InvalidOperationException("No task specified by the query string.");
    	
    	RemoveTaskFromMilestoneStrategy strategy = new RemoveTaskFromMilestoneStrategy();
    	
    	strategy.Remove(taskID, milestoneID);
    	
    	Result.Display(Language.TaskRemovedFromMilestone);
