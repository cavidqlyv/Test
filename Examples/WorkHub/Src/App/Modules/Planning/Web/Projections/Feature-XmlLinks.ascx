...
<script runat="server">
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            <div class="Heading1">
                        <%= Language.FeaturesXml %>
                    </div>
              
                        <ss:Result ID="Result1" runat="server">
                        </ss:Result>
                        <p>
                            <%= Language.FeaturesXmlIntro %>
                        </p>
                    <H2><%= Language.FeaturesXmlPages %></H2>
                    <ul><li><a href="<%= new ExternalUrlCreator().CreateExternalXmlUrl("Index", "Feature") %>" target="_blank"><%= Language.AllFeaturesInProject %></a></li>
                    </ul>