...
<script runat="server">
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            <div class="Heading1">
                        <%= Language.ProjectEntitiesXml %>
                    </div>
                        <ss:Result ID="Result1" runat="server">
                        </ss:Result>
                        <p>
                            <%= Language.ProjectEntitiesXmlIntro %>
                        </p>
                    <H2><%= Language.ProjectEntitiesXmlPages %></H2>
                    <ul><li><a href="<%= new ExternalUrlCreator().CreateExternalXmlUrl("Index", "ProjectEntity") %>" target="_blank"><%= Language.AllProjectEntitiesInProject %></a></li>
                    </ul>