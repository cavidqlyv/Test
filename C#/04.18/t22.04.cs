...
class Program
{
    ...
    static void Main()
    {
        var Mail = from x in debtors
                   where x.Email.IndexOf("rhyta.com") != -1 || x.Email.IndexOf("dayrep.com") != -1
                   select x;
        ...
        var Year = from x in debtors
                   where x.BirthDay.Year > 1982 && x.BirthDay.Year < 1992
                   select x;
        ...
        var N18 = from x in debtors
                  where x.FullName.Length > 18 && x.Phone.IndexOf("7") != x.Phone.LastIndexOf("7")
                  select x;
        ...
        var Winter = from x in debtors
                     where x.BirthDay.Month == 12 || x.BirthDay.Month == 1 || x.BirthDay.Month == 2
                     select x;
        ...
        var Avg = from x in debtors
                  where x.Debt > debtors.Average(y => y.Debt)
                  select x;

        var AvgSurname = from x in debtors
                         orderby x.FullName
                         select x;

        var AvgDebt = from x in debtors
                      orderby x.Debt
                      select x;
