...
class Program
{
    ...
    static List<Debtor> debtors = new List<Debtor>
    {
            ...
            new Debtor("Pamela H. Beauchamp",   DateTime.Parse("November 20, 1959"),    "801-559-7637", "PamelaHBeauchamp@jourrapide.com",  "3239 Tori Lane Salt Lake City, UT 84104",          6588)
    };

    static void Main()
    {

        var Mail = debtors.Where(x => x.Email.IndexOf("rhyta.com") != -1 || x.Email.IndexOf("dayrep.com") != -1).ToList();
        Console.WriteLine("rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag : \n\n");
        foreach (var item in Mail)
        {
            Console.WriteLine($"{item.FullName} {item.Email} {item.Debt}");
        }
        Console.WriteLine("\n\n================================================================================\n");

        var Year = debtors.Where(x => x.BirthDay.Year > 1982 && x.BirthDay.Year < 1992).ToList();
        Console.WriteLine("\nYashi 26 - dan 36 - ya qeder olan borclulari cixartmag\n\n");

        foreach (var item in Year)
        {
            Console.WriteLine($"{item.FullName} {item.BirthDay} {item.Email} {item.Debt}");
        }

        Console.WriteLine("\n\n================================================================================\n");

        var D5000 = debtors.Where(x => x.Debt < 5000).ToList();

        Console.WriteLine("\nBorcu 5000 - den cox olmayan borclularic cixartmag\n\n");

        foreach (var item in D5000)
        {
            Console.WriteLine($"{item.FullName} {item.BirthDay} {item.Email} {item.Debt}");
        }
        ...
        var N18 = debtors.Where(x => x.FullName.Length > 18 && x.Phone.IndexOf("7") != x.Phone.LastIndexOf("7"));
        ...
        {
            Console.WriteLine($"{item.FullName} {item.BirthDay} {item.Email} {item.Phone} {item.Debt}");
        }

        Console.WriteLine("\n\n================================================================================\n");

        var Winter = debtors.Where(x => x.BirthDay.Month == 12 || x.BirthDay.Month == 1 || x.BirthDay.Month == 2);
        ...
        var Avg = debtors.Where(x => x.Debt > debtors.Average(y => y.Debt));

        var AvgSurname = debtors.OrderBy(x => x.FullName);

        var AvgDebt = debtors.OrderByDescending(x => x.Debt);
        ...
        foreach (var item in AvgSurname)
        {
            Console.WriteLine($"{item.FullName} {item.BirthDay} {item.Email} {item.Phone} {item.Debt}");
        }
        ...
        var P8 = debtors.Where(x => x.Phone.IndexOf("8") == -1).Select(x => new { Surname = x.FullName.Split(' ').Last(), Age = 2018 - x.BirthDay.Year, Debt = x.Debt });
        ...
        {
            Console.WriteLine($"{item.Surname} {item.Age} {item.Debt}");
        }

        Console.WriteLine("\n\n================================================================================\n");

        var N3 = debtors.Where(x => x.FullName.Count(f => f == 'a') >= 3);
        ...
        int AvgYear = (int)debtors.Average(x => x.BirthDay.Year);
        ...
        var Top5 = debtors.OrderByDescending(x => x.Debt).Take(5);
        ...
        var Sum = debtors.Sum(x => x.Debt);
        Console.WriteLine("\nButun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq\n\n");
        Console.WriteLine(Sum);

        Console.WriteLine("\n\n================================================================================\n");

        var War = debtors.Where(x => x.BirthDay.Year <= 1945);
