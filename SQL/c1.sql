...
select count(gs.student_id) , gs.student_id as StudentGroupCount
FROM GroupStudnet gs
    join Student s on s.id = gs.student_id
GROUP by  gs.student_id
HAVING count(gs.student_id) = (Select max(q.StudentGroup)
from ( select count(gs.student_id) as StudentGroup
    FROM GroupStudnet gs
...
SELECT table_catalog [db], table_schema [schema], table_name name, table_type type
FROM INFORMATION_SCHEMA.TABLES
GO

-- List columns in all tables whose name is like 'Student'
SELECT 
...
WHERE tbl.TABLE_TYPE = 'BASE TABLE' and tbl.TABLE_NAME like '%Student%'
GO